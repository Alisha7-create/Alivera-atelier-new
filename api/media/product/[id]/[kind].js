...
export default async function(req, res) {
  const field = req.params.kind === 'chart' ? 'size_chart_url' : 'image_url';
  const { rows } = await db.query(`SELECT ${field} AS url FROM products WHERE id = $1 LIMIT 1`, [req.params.id]);
