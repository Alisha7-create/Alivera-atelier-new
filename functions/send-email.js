export async function onRequestPost(context) {
  try {
    ...
    if (status === 'Under Production') {
      ...
      messageBody = `A refund of ₹${amount} for your order of <strong>${dressDetails || 'Custom Garment'}</strong> has been processed and should reflect in your account within 5-7 business days.`;
    ...
    // High-fashion editorial email template for the customer
    ...
    <div style="background-color: #050505; color: #f5f5f4; font-family: 'Plus Jakarta Sans', Arial, sans-serif; padding: 40px; max-width: 600px; margin: 0 auto; border: 1px solid rgba(191, 149, 63, 0.3);">
        ...
        <div style="background-color: #0c0a09; border: 1px solid rgba(191, 149, 63, 0.3); padding: 16px; margin: 24px 0; font-size: 12px;">
            <p style="color: #fcf6ba; font-weight: bold; text-transform: uppercase; margin: 0 0 8px 0; letter-spacing: 1px;">--- Bill & Tax Details ---</p>
            <p style="margin: 4px 0; color: #d6d3d1;"><strong>GSTIN Issuer:</strong> 08CBPPJ9205G1ZE</p>
            <p style="margin: 4px 0; color: #d6d3d1;"><strong>Total Amount:</strong> ₹${amount}</p>
    ...
    // 2. Send the notification to your atelier inbox with the `from` field set to the customer's email
    await env.EMAIL.send({
      to: "contact@aliveraatelier.in",
      from: to, // Sent from customer's email address
      subject: `[New Order Alert] #${shortId} - ₹${amount}`,
      html: `
    ...
    return new Response(JSON.stringify({ success: true, message: "Customer and admin emails sent successfully" }), {
      headers: { 
