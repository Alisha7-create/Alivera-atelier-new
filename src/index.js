...
function findRoute(method, path) {
  ...
  if (
    ...
    return {
      handler: storageFile,
      params: {
        key: path.slice('/api/storage/'.length)
      }
    };
  }
...
async function getUser(request) {
  ...
  const r = await db.query(
    'SELECT id,email,name,active FROM users WHERE id=$1 LIMIT 1',
    [session.id]
  );
...
async function dispatch(request, env) {
  ...
  if (route) {
    ...
    if (
      ...
      !route.handler.methods.includes(
        request.method
      )
    ) {
      return new Response(
