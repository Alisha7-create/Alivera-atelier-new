...
function findRoute(method, path) {
  ...
  if (exactRoutes[key]) return { handler: exactRoutes[key], params: {} };

  // Wildcards & parameters
  if (method === 'GET') {
    if (path.startsWith('/api/storage/')) return { handler: storageFile, params: { key: path.slice('/api/storage/'.length) } };
    if (path.startsWith('/api/products/')) return { handler: productSlug, params: { slug: decodeURIComponent(path.slice('/api/products/'.length)) } };
    
    let p = matchPath('/api/media/product/:id/:kind', path);
    if (p) return { handler: productMediaKind, params: p };

    p = matchPath('/api/media/product/:id', path);
    if (p) return { handler: productMedia, params: p };
  }
...
async function dispatch(request, env) {
  ...
  if (env.ASSETS) {
    ...
    if (path === '/account' || path === '/my-account') assetPath = '/my-account.html';
