...
const titles={confirmed:'ORDER CONFIRMED',processing:'ORDER BEING PREPARED',shipped:'ORDER SHIPPED',delivered:'ORDER DELIVERED',cancelled:'ORDER CANCELLED'};
const messages={
...
export default async function(req,res){
 if(req.method==='GET'){const {rows}=await db.query("SELECT * FROM orders ORDER BY created_at DESC");for(const o of rows){const r=await db.query("SELECT product_name AS name,size,quantity,line_total,custom_fit FROM order_items WHERE order_id=$1 ORDER BY id",[o.id]);o.items=r.rows;}return res.json(rows)}
 const b=req.body||{};const {rows:before}=await db.query('SELECT * FROM orders WHERE id=$1 LIMIT 1',[b.id]);const previous=before[0];
 const {rows}=await db.query("UPDATE orders SET status=COALESCE($1,status),payment_status=COALESCE($2,payment_status) WHERE id=$3 RETURNING *",[b.status||null,b.payment_status||null,b.id]);const order=rows[0];if(!order)return res.status(404).json({error:'Order not found.'});
 if(previous&&previous.status!==order.status&&titles[order.status]){
  ...
  try{await sendResend(order.email,`${titles[order.status]} · ${order.order_number} · Alivèra Atelier`,shell(body),assets.attachments);}catch(e){console.log(`Status email (${order.status}) failed`,e?.message)}
 }
